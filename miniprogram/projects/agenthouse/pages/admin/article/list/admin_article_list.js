...
Page({
	...
	onLoad: async function (options) {
		...
		wx.setNavigationBarTitle({
			title: projectSetting.ARTICLE_NAME + '-管理',
		});
		this.setData({
			ARTICLE_NAME: projectSetting.ARTICLE_NAME
		});
	...
	bindMoreTap: async function (e) {
		...
		let order = this.data.dataList.list[idx].ARTICLE_ORDER;
		let orderDesc = (order == 0) ? '取消置顶' : '置顶';

		let vouch = this.data.dataList.list[idx].ARTICLE_VOUCH;
			...
			success: async res => {
				switch (res.tapIndex) {
					case 0: { //预览
						...
						wx.navigateTo({
							url: '../../../article/detail/article_detail?id=' + id,
						});
						break;
					}
					case 1: { //置顶 
	...
	_setSort: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/article_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'ARTICLE_ORDER', sort);
				this.setData({
	...
	_setVouch: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/article_vouch', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'ARTICLE_VOUCH', vouch);
				this.setData({
					dataList: this.data.dataList
				});
				pageHelper.showSuccToast('设置成功');
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/article_del', params, opts).then(res => {
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/article_status', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'ARTICLE_STATUS', status, '_id');
				this.setData({
					dataList: this.data.dataList
				});
				pageHelper.showSuccToast('设置成功');
