...
Page({
	...
	bindCompanyTap: function (e) {
		...
		pageHelper.showConfirm('确认选择该小区？', cb);
	...
	_getSearchMenu: async function () {
		...
		let sortItems1 = [{ label: '分类', type: '', value: '' }];
		let cateList = projectSetting.CATE_CATE;
		for (let k = 0; k < cateList.length; k++) {
			sortItems1.push({ label: cateList[k], type: 'cate', value: cateList[k] })
		}

		let sortItems2 = [{ label: '区域', type: '', value: '' }];
		let areaList = projectSetting.AREA;
		for (let k = 0; k < areaList.length; k++) {
			sortItems2.push({ label: areaList[k], type: 'area', value: areaList[k] })
		}

		let sortItems3 = [{ label: '地铁', type: '', value: '' }];
		let lineList = projectSetting.CATE_LINE;
		for (let k = 0; k < lineList.length; k++) {
			sortItems3.push({ label: lineList[k], type: 'line', value: lineList[k] })
		}
		...
		let sortMenus = [
			...
			{ label: '房源数▽', type: 'sort', value: 'CATE_CNT|desc' },
